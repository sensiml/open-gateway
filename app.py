...
from flask import Flask, Response, stream_with_context, url_for, render_template, redirect, request
import time
from json import dumps
import random, struct
import math
from forms import SerialPortForm
from data import _generate_samples, get_port_info, pack_data, check_for_config, _get_serial_data, send_connect, _generate_samples
...
_config = {"sample_rate": 119, 
           "serial_port": "/dev/ttyACM0", "samples_per_packet": 10, "column_location": {"AccelerometerY": 1, "AccelerometerX": 0, "GyroscopeZ": 5, "GyroscopeY": 4, "GyroscopeX": 3, "AccelerometerZ": 2}}
...
def serial_port():
    ...
    return render_template('serialport.html', form=form, serial_port_list=get_port_info())
...
def stream():
    ...
    def gen():
        if app.config['TEST']:
            ...
            while True:
                sample_data, index = pack_data(data, byteSize, app.config["CONFIG_SAMPLES_PER_PACKET"], index)
                yield sample_data
                time.sleep(delay)

        else:
            while True:
                yield _get_serial_data(app.config['SERIAL_PORT'], len(app.config["CONFIG_COLUMNS"]) *app.config["CONFIG_SAMPLES_PER_PACKET"] )
